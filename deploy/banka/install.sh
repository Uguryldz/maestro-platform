#!/usr/bin/env bash
# =============================================================================
# Maestro — banka içi (internetsiz) kurulum
# BUNDLE_VERSION: 2026-08-27
# =============================================================================
# Kullanım (bu klasörde):
#   ./install.sh          # interaktif
#   ./install.sh --yes    # sorusuz (otomasyon için)
#
# Ön koşul: docker + compose v2 kurulu, imajlar `docker load` ile yüklenmiş,
#           .env doldurulmuş.
#
# Bu betik İNTERNETE ÇIKMAZ. Hiçbir adımı imaj indirmez, sürüm sorgulamaz,
# telemetri göndermez — banka sunucusunun dışarıya yolu yoktur.
#
...
echo "=================================================="
echo "  Maestro — banka içi kurulum"
...
# `. ./.env` yukarıda BUNDLE_VERSION'ı .env'deki değerle EZDİ; bu yüzden dosyadan
# ayrıca okunur ve betiğin kendi sürümüyle karşılaştırılır.
...
#
# ⚠ Bu liste KISADIR ve kısalığı bilinçlidir: Jira jetonu, GitHub jetonu, model
#   anahtarı ve bot e-postası burada YOKTUR çünkü panelden girilir/öğrenilir.
#   Bu adlardan birini listeye eklemek, panelin zaten topladığı bir sırrı
#   ikinci bir yerde daha zorunlu kılmak olurdu — ve iki yer birbirinden
#   habersiz olduğu sürece ayrışabilirler. Eski paket tam bunu yapıyordu:
#   banka, panelin hiç okumayacağı sahte bir model anahtarı uydurmak zorunda
#   kalıyordu.
...
# JIRA_TOKEN_REF / LLM_API_KEY_REF bu listede ARTIK YOK. İkisi de sır
# ADRESİDİR, sır değeri değil; varsayılanları compose'da ve
...
#
# MAESTRO_SECRET_KV_JIRA__WEBHOOK ise tek kalan zorunlu SIRDIR ve panele
# taşınamaz: dışarıdan gelen, henüz doğrulanmamış isteği doğrular ve kontrol
# fail-closed'dır — sır boşsa her webhook 401 alır ve akış hiç başlamaz.
EKSIK_VAR=()
for v in "${ZORUNLU[@]}"; do
  [[ -n "${!v:-}" ]] || EKSIK_VAR+=("$v")
done
...
# MAESTRO_PROFILE kaldırıldı: sırların ve belgelerin NEREDE durduğu artık tek
# başına NODE_ENV'den türetilir (apps/deploy/src/env.ts). İki ayrı değişken
# olduğu sürece birbiriyle çelişebiliyordu ve çelişkinin bedeli açılışta ölen
# bir yığındı — üstelik log'a bakmayan biri için "kurulum bitti" görünüyordu.
#
# Profil ARTIK ürünün ne yapabileceğine karışmaz. Kod hattı (scm) her kurulumda
# kuruludur; hangi akışın çalışacağı ticket başına panelden seçilir.
info "Altyapı katmanı…"
if [[ "${NODE_ENV}" == "production" ]]; then
  # Vault + S3: üretim katmanı. Biri eksikse süreç açılışta ölür, o yüzden
  # burada — yığın başlamadan — söylüyoruz.
  ...
  # Sırlar .env'de, belgeler Postgres'te. Pilotun ve bugünkü sahanın hâli.
  ok "Altyapı: development (sırlar .env'de, belgeler Postgres'te)."
fi

# ── 5) Motorun Jira kimliği ──────────────────────────────────────────────────
# Bu boşsa keşif `assignee = currentUser()` ile çalışır: jetonun sahibi kimse
# onun talepleri işlenir. Panelden kurulan dinleme kuralları ise başka bir
# kimliğe bakar ve HİÇ EŞLEŞMEZ — hiçbir yerde hata görünmeden.
...
# Sürücü değişken adını referanstan TÜRETİR (kv/jira#token →
# MAESTRO_SECRET_KV_JIRA__TOKEN, '#' çift alt çizgi). Eski paketlerdeki
# elle yazılmış adlar hiç aranmıyordu: sürücü "secret not found" derken
# operatör .env'de değeri dolu görüyordu.
...
# healthcheck'te ve studio-nginx.conf'taki proxy_pass satırında SABİTTİR, süreç
# ise başka bir portu dinlemeye başlar. Bu yüzden 7001 dışında bir değer
# gördüğümüzde sessizce yok saymak yerine durup söylüyoruz — yok saysaydık
# operatör ayarın işe yaradığını sanmaya devam ederdi.
if [[ -n "${BFF_PORT:-}" && "${BFF_PORT}" != "7001" ]]; then
  ...
  err "  aynı sayı healthcheck ile studio-nginx.conf'ta da sabit durur. Bu"
  err "  satır dosyada kaldığı sürece değiştirilebilir görünür; değiştirildiği"
  err "  anda bff hiç 'healthy' olmaz ve studio hiç başlamaz."
...
if [[ -n "${NODE_EXTRA_CA_CERTS:-}" ]]; then
  ...
  if [[ ! -f "$CA_LOCAL" ]]; then
    ...
    err "  Banka kök CA'sını '${CA_BUNDLE_DIR:-./certs}' klasörüne koyun."
    die "CA dosyası eksik."
  fi
  ok "Kurumsal CA bulundu: ${CA_LOCAL}"
else
  warn "NODE_EXTRA_CA_CERTS boş — Jira/TFS sertifikası kurum CA'sıyla imzalıysa"
  warn "  bağlantılar 'unable to verify the first certificate' ile düşer."
fi

# LDAP seçiliyse KODUN OKUDUĞU alanlar dolu mu.
# Dikkat: LDAP_USER_BASE_DN. Eski paketlerde `LDAP_BASE_DN` yazıyordu ve o adı
# hiçbir şema okumuyordu — LDAP dolu görünüp hiç çalışmıyordu.
...
# ── 8) İmajlar yerelde mi ────────────────────────────────────────────────────
info "İmajlar kontrol ediliyor…"
EKSIK=0
for img in "${MAESTRO_NODE_IMAGE}" "${MAESTRO_STUDIO_IMAGE}" \
           "${POSTGRES_IMAGE}" "${REDIS_IMAGE}" "${TEMPORAL_IMAGE}"; do
...
if [[ "$EKSIK" -eq 1 ]]; then
  err "Eksik imaj var. İnternetsiz sunucuda build yapılamaz."
  err "  Çözüm:  gunzip -c maestro-imajlar-<TAG>.tar.gz | docker load"
  err "  (kurum registry'si varsa: docker compose pull)"
  die "İmajları yükleyip tekrar çalıştırın."
fi

# `latest` etiketi geri almayı imkânsızlaştırır.
...
# ── 9) İmaj sağlığı (bayat/bozuk imaj erken yakalansın) ──────────────────────
...
if docker run --rm --entrypoint sh "${MAESTRO_NODE_IMAGE}" -c "openssl version" >/dev/null 2>&1; then
  ...
  warn "  İmajda OpenSSL YOK. Prisma sürüm tahmini yapar; sorun çıkarsa"
  warn "  imajı yeniden build edin (README §1.3)."
fi
...
# (0011_failed_run_is_terminal, 0011_listening_rules). Sayıyı en yüksek
# öneke bakarak tahmin etmek bu yüzden yanlış olur.
#
# Bu sayı elle yazılıdır ve bir kez sessizce bayatladı: 0021 eklendiğinde satır
# 21'de kaldı ve `-ge` karşılaştırması panel-öncesi bir imajı yeşil geçirdi.
# apps/deploy/test/banka-bundle.test.ts artık bu sayıyı depodaki klasör
# sayısıyla karşılaştırır — yeni migration eklendiğinde test kırmızıya düşer
# ve satır güncellenmeden paket çıkamaz.
#
...
if [[ "${MIG_SAYI}" -ge "${MIG_BEKLENEN}" ]]; then
  ...
  die "Güncel koddan yeniden build edin."
fi
...
#
# Kalıp düzeltildi, ama düzeltilen şey KAYNAKTIR; elinizdeki imaj ondan önce
# build edilmiş olabilir ve dışarıdan bakınca ikisi ayırt edilemez. Sızıntı
# yalnızca imajın İÇİ listelenerek görülür, ki kimse bunu rutin olarak yapmaz.
# Bu yüzden kurulum onu operatör adına yapar: imaj bir kez bankaya girdikten
# sonra geri alınamaz.
if docker run --rm --entrypoint sh "${MAESTRO_NODE_IMAGE}" \
  ...
  err "  Bu imaj bankaya GÖTÜRÜLMEMELİDİR. '.dockerignore' içindeki kalıbın"
  err "  '**/.env' olduğu güncel koddan yeniden build edin ve bu etiketi silin."
...
# ── 10b) ORTAM ÖN DOĞRULAMASI — şemanın kendisiyle ───────────────────────────
# Yukarıdaki 9 adım 24 değişkeni tek tek kontrol eder ve YİNE DE açılamayan bir
...
# sürece ayrışırlar, ve ayrıştıkları gün fark ediliş biçimi budur: 24 yeşil
# tik, ardından `migrate` konteynerinin tek satırlık ölümü.
#
...
#
# Hiçbir servise, veritabanına ya da ağa dokunmaz: yalnızca ortamı okur.
...
if ! ONDOGRULAMA_CIKTI=$(docker compose run --rm --no-deps \
  ...
  err "  import ettiği dosya imajda yoktur. İmajın bu paketle aynı sürümden"
  err "  geldiğini doğrulayın (imaj apps/deploy/dist/env.js taşımalı)."
  err ""
...
# Konteynerler ADIYLA değil, compose'a SORULARAK bulunur.
#
# Burada eskiden `docker inspect maestro-bff` yazıyordu ve bu, sabit
# `container_name:` alanlarına bağlıydı. O alanlar kaldırıldı (aynı sunucuda
# ikinci bir kurulumu imkânsız kılıyorlardı), dolayısıyla konteynerin adı artık
# yığın adına göre değişir: `maestro-bff-1`, `kabul-bff-1`, …
#
# Sabit ad bırakılsaydı arıza sessiz olurdu: `docker inspect` bilinmeyen adda
# hata verir, `|| echo ""` onu yutar, karşılaştırma hiçbir zaman "healthy"
# olmaz ve betik 150 saniye bekledikten sonra sapasağlam bir yığın için
# "servisler hazır olmadı" uyarısı basardı. Operatör de çalışan bir kurulumu
# bozuk sanarak log okumaya başlardı.
HAZIR=0
...
warn "  4) Ayarlar & bağlantılar → kurum içi model sunucusunu ekleyin ('Test et' yeşil olmalı)"
warn "  5) /setup sihirbazını çalıştırın"
warn "  6) Jira webhook'unu kaydedin (docs/jira-webhook-kurulum.md)"
warn "  7) bot hesabı 'maestro-admins' grubunda OLMASIN (tek başına dört-göz kuralını geçer)"
echo
